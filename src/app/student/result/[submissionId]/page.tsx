...
export default function StudentResultParams({ params }: { params: Promise<{ submissionId: string }> }) {
    const { submissionId } = use(params);
    return <StudentResult submissionId={submissionId} />
}

function StudentResult({ submissionId }: { submissionId: string }) {
    ...
    if (loading) return <div className="p-8 text-center">Generated Analysis Report...</div>;
    if (!submission) return <div className="p-8 text-center">Submission not found or access denied.</div>;
